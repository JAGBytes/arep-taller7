...
public final class ApiResponses {
    ...
    public static APIGatewayProxyResponseEvent notFound(String m){ return build(404, Map.of("message", m)); }
    public static APIGatewayProxyResponseEvent conflict(String m){ return build(409, Map.of("message", m)); }

    private static APIGatewayProxyResponseEvent build(int code, Object body){
        try {
            return new APIGatewayProxyResponseEvent().withStatusCode(code).withHeaders(baseHeaders()).withBody(M.writeValueAsString(body));
        } catch (Exception e) {
            return new APIGatewayProxyResponseEvent().withStatusCode(500).withHeaders(baseHeaders()).withBody("{\"message\":\"serialization error\"}");
        }
    }
